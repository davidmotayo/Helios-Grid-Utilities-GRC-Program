...
A single, consolidated risk register spans both domains, with a domain tag (IT / OT / Shared) on every entry so that reporting can be sliced by domain while governance remains unified. Each entry records: Risk ID, Domain, Asset/Process affected, Threat, Vulnerability, Likelihood (1--5), Impact (1--5, per the scale above), Inherent Risk Score, Existing Controls, Residual Risk Score, Risk Owner, Treatment Plan, Target Date, and Status. The entries below are drawn from the full register (maintained as a living document outside this policy) and show current inherent risk, ahead of the treatment plans defined there.

  ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  **Risk ID**   **Domain**   **Risk Description**                                                                      **Likelihood**   **Impact**   **Inherent**   **Owner**
  ------------- ------------ ----------------------------------------------------------------------------------------- ---------------- ------------ -------------- ------------------
  R-03          OT           Unpatched, unsupported legacy OT controllers/RTU firmware exploitable via newly networked remote monitoring links      4                5            Critical       OT Engineering Director

  R-02          OT           Poorly inventoried vendor remote-access pathway into substation network                   5                4            Critical       Third-Party Risk Manager

  R-06          IT           Fragmented IAM allows orphaned accounts to persist across legacy and cloud systems        4                4            Critical       IAM Lead

  R-04          Shared       No consolidated IT/OT asset inventory delays incident scoping and regulatory reporting    4                4            Critical       GRC Analyst / IT Asset Mgmt

  R-05          OT           Smart meter fleet (multi-vendor) introduces heterogeneous, hard-to-patch attack surface   5                3            Critical       Smart Grid Program Manager

  R-10          IT           Customer portal vulnerable to credential-stuffing due to absent MFA enforcement           4                3            High           IT Security Manager
  ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Poorly inventoried vendor remote-access pathways into OT are treated as one of the highest-priority risks in this program (see R-02). The following controls are mandatory:
